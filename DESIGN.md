...
- **Slat Edge** (slatHi, #232329): hairline rules, borders, slider handles at rest.
...
- **Dim Silver** (dim, #8A8878): muted labels, unselected chip names, the receipt body — the one de-emphasis step below lit text.

Amber light also appears as alpha steps of the same hex — rgba(255,176,46,0.12/0.16/0.18/0.22/0.32) for glow shells, chip focus tints and the armed slider — and chalk as rgba(232,230,220,0.03–0.25) for the sign panel, sprocket strips and leader ring. These are the lamp at intensities, not new colors. The one surface outside the palette is the QR card on the import screen, which renders white because scanners read dark-on-light far more reliably; it is a machine target, not part of the booth. When there is no address to encode, the same card renders unlit — slat fill, slatHi border, the reel standing in where the code would be, and the cyan warning taking the URL's place beneath it: the booth's own word for "no picture here", never a dashed box. The phone-side upload page it opens is the same kind of machine target, set in the phone's own system UI at phone sizes.
...
- **Headline** (Archivo 800, 32px, −0.02 to −0.03em): each screen's header title, with exactly one amber word — the board's is the wordmark what.watch (the dot amber), the account's "your list" and the import's "sync your list" (the word "list" amber). The verdict carries no header; its chrome is the sprocket frame and gate light.
- **Action** (Archivo 800, 28px, +0.12em, caps): button labels only, and never a digit — versions live in the ledger. One exception: "IMDb" keeps the brand's own casing; a label that is a proper noun is not misspelled to satisfy the caps recipe.
- **Body/labels** (Chivo Mono 400, 18–30px, lh 1.25, +0.02 to +0.2em): the workhorse size is 24px for tracked-caps chrome labels; 26px mixed-case plot, changelog and step text (plot line-height 42); 22–23px for tags, log lines and quiet notes; 18px for the corpus aside and the TMDB credit. Tracked caps step through four widths only: 0.02em on selections, 0.08em on receipts, 0.1em on warnings and meta lines, 0.2em on labels. One tier step: the genres block head sits at 0.75 alpha of the dim ink — the fine-tuning tier of the board, still 3:1 on the ground — while every entry-point head (Type, the three ranges) reads full dim.
- **Numbers** (Chivo Mono 700, 24–44px): the score at 44 (its star drawn as SVG beside the digits, never a typed glyph), the update version at 40, handle values, the progress percent and the receipt count at 26. Numbers set in the mono; groups of thousands separated by spaces (534 836) wherever a count is shown whole. One deliberate compression: slider handles and the receipt abbreviate (534K, 1M+) because a 124px handle cannot hold nine digits.
...
Every screen sits on the board ground inside a 5% overscan inset on all four edges (percentage padding resolves against the width, so every edge carries the same 96px band), so nothing lands on a bezel. The board is a 7-column grid: column gap s(12), cell width computed from the inset content width (cell = (contentWidth − 6 x gap) / 7), a cell spanning n columns is span(n) = n·cell + (n−1)·gap. Cells are sized, never flexed. Spacing rhythm on the board: header padding-bottom s(8) over a hairline on every screen that has a header (the verdict opens frameless); the cadence is gap contrast — s(4) inside a block, s(14) between blocks (blocks padding-top s(2)); each block gap s(4) under a 28px block head; a hairline above every dock (padding-top s(2)); wider gaps s(18–28) on the verdict. The verdict main column spans 5, the poster 2 (2:3 aspect); its action buttons share the main's five columns equally (three when Plex matched, two otherwise); the pick button on the board is the gate bar — the full seven columns on its own bottom row, s(80) like every button, the one control on the board that is also the whole dock; reset filters rides the header as a utility chip in the doors' grammar; the presets screen ("your presets") keeps its rows on the same grid: a span(4) card — name in chalk bold, summary in the receipt's grammar and its exactness contract, sliced by character budget with an amber "+N more", never an ellipsis — beside span(1) replace, rename and delete ghosts, with "+ keep this board" above (its live preview is the receipt line itself, run through the same budget slice — never a tail ellipsis) and back in the dock; delete and replace each answer with a four-second "— undo" pressable in the notice slot, not a confirm, and the affordance retires with the note; BACK cancels a rename while DONE commits — back is the way out, never the way in; a keep of the board already at the top of the list answers "this board is already kept"; the presets door is a header chip in the account door's grammar; an offered update turns the check chip itself — amber border, amber tracked-caps "install update" — and pressing it opens the install modal centered over the darkened board (no lamp beside the doors: a chip's amber is state, never decoration); the account screen is one centered span(4) plate — device tag, watched count, and the import door, updates living on the board; the import dock shows three span(2) buttons — paste mode replaces pick mode, because four would be eight columns.

The count is not a board control — the dock holds the gate bar and nothing else. The exact number lives where the decision is: the verdict receipt states "N left" in bold sodium, and the board speaks the same count aloud on change for TalkBack. On the board itself the count has exactly one visible form: a settled zero disables the pick (reached-but-inert, never a grey trap). The header corner keeps only the wordmark and the doors (presets, import, reset — utilities, not warnings). Focus order is a contract, not a suggestion: down is straight down, short rows are inert at their own edges, and full-width rows are wired explicitly because Android's FocusFinder scores by centre distance.
...
### Range Slider
One focus cell, not two handles, and focus is the mode: the focused row's amber end takes every left/right immediately — no arm press, nothing to exit — while up/down is navigation at all times, because values apply as they move and there is nothing to commit. OK moves the amber mark to the other end without moving a value (the genre chip's own cycle-the-cell's-state idiom), and the mark is sticky per axis across focus leaves, so a return trip resumes the end you were tuning. A direction the amber end cannot take — its axis wall, or its partner one step away — hands the step, and the mark, to the other end; the handoff is evaluated once at a press's first notch, never mid-hold, because a hold must never silently move both ends. Arrows adjust after the OS's own long-press dead zone (~300 ms), then ticks ramp from 100 ms toward a 35 ms floor (0.87 decay per notch) — long traverses accelerate the way native D-pad UIs do. The slat tints amber (0.17 focused, 0.32 driving) instead of taking a ring — a ring around a full-width element reads as an alarm — and at focus the amber end wears dim chevrons (sodiumDim on slatLit) so the next press's target reads before it moves. Handles (124 x 44) sit slatHi at rest, slatLit when the row has focus, sodium when live; the end the arrows are not driving dims while they drive. Track: boardLo groove, hairline slatHi edge, sodium fill at 0.22 for a whole axis at rest (an untouched range is not a choice), 0.45/0.75/1 by state once narrowed.

### Inputs / Fields
Slat fill, slatHi 2px border, radius s(3), chalk mono text (30px, height s(64)), dim placeholders. Remote typing is miserable, so forms are exactly two fields. A TV TextInput that holds focus eats the D-pad for its caret, so fields are rows first — OK opens the row for typing (sodium border while open), DONE or back closes it and focus returns to the row. Every screen owns its initial focus so arrows always have an anchor.

### Cards / Containers
Slat fill, slatHi 2px border, radius s(3) — the install modal is the instance: centered over a veil of boardLo at 0.86 alpha, internal padding s(24) and gap s(14), the version in amber mono bold 40, the whole changelog in chalk mono 26 (a scroll takes over once a release outgrows the sheet), progress as a sodium fill in a slatHi track. No shadows; the border is the card.

### Receipt (leader tape)
Full-width strip (height s(76), tape fill, slatHi 2px border, radius s(3)) along the verdict's bottom, carrying its facts in two 24px lines: line one the kinds and ranges, line two the genre marks. The tape shares the counter's exactness contract — the mono face makes a segment's width its character count, so genres are sliced against a character budget and the cut is stated as an amber "+N more", never an ellipsis. The facts read in chalk with only the "·" separators dim (dim facts on the tape's near-black failed at three metres), "+ genre" in sodium, "− genre" in cyan struck through, and the remaining count in bold sodium at the right end. The accessibility label carries the full genre list regardless of the slice. The tape doubles as the way back to the board; focus is a sodium border only.
...
- **Do** move in hard steps (240 ms thread-up cuts, constant 100 ms key repeat) and skip motion whole under reduce-motion.
